...
\graphicspath{{./Figures/}{./figures/}{../host_data/results/reward_fix/figures/}}
...
\begin{abstract}
Deep Reinforcement Learning (DRL) has been proposed for learning adaptive routing policies, but reported gains are typically measured only against shortest-path forwarding, on the training topology, and with the training operating point left uncalibrated. We present a controlled comparison of Multi-Agent Deep Deterministic Policy Gradient (MADDPG) routing variants spanning two design axes --- the \emph{critic domain} (centralised vs.\ local) and Graph Neural Network (GNN) state encoding --- trained at their intended stress operating point (concentrated demand at $2\times$ load) on a real 86-node service-provider topology, and evaluated against a demanding baseline set: an equivalent shortest-path control plane, uniform random path spreading, and a greedy least-utilised-path heuristic on traffic-paired episodes. Under concentrated demand the centralised critic dominates: it beats shortest path at every offered load (by $4.5$--$15.1$ percentage points of delivery) and its local-critic counterpart in every matched comparison --- a seed-stable effect realised by trading longer paths for wider load spread. The honest baselines relocate the contribution: random spreading out-delivers the best learned policy by ${\sim}3$\,pp, but the policy holds the only interior point of the delivery/latency frontier, with unanimously lower tail delay. Under randomised out-of-distribution link failures the ranking inverts: the learned policy degrades gracefully ($-10$\,pp at eight failed links) while random spreading collapses ($-81$\,pp). GNN encoding costs delivery on the intact topology across all architectures tested, and a training-regime ablation shows calibration is worth more delivery than any architectural choice except the critic domain. The results give mechanism-grounded guidance for selecting, training, and honestly evaluating MADDPG routing architectures on service-provider networks.
\end{abstract}
\IEEEpubidadjcol
\begin{IEEEkeywords}
...
Our study examines seven architectural variants built from two critic domains (centralised and local) and two value-head designs (simple and duelling Q-networks), each evaluated with and without GNN integration, on a real 86-node service-provider topology with a provider-edge, $K$-shortest-path forwarding model (e.g.\ EVPN). Performance is assessed under offered-load scaling of concentrated (hotspot) demand, under randomised multi-link failures of increasing severity, and in targeted dual-link failure scenarios, using packet delivery, tail latency, path length, and link-utilisation statistics.

The study is deliberately scoped to the \emph{service-provider topology class}, and this scoping is a feature of the research question rather than a threat to it. The evaluation topology is a real hierarchical SP network, and the two structural properties on which the headline behaviours rest --- hop-diverse alternative paths between provider edges, and core capacity provisioned with headroom for peak demand --- are not idiosyncrasies of the instance but design invariants of the class: operators build SP networks precisely for path redundancy and peak provisioning. The question this paper answers is therefore not whether MADDPG routing wins on an arbitrary graph, but whether this family of methods adds operational value on the networks it would actually be deployed in --- and Section~\ref{subsec:topology-scope} makes explicit which conclusions are predicted to transfer across the class and which should be re-verified on sparser or capacity-tight designs.

The contributions of this work are fourfold. First, we provide a systematic comparison of centralised and local critic architectures within the MADDPG framework and show that, once training is performed at the intended stress operating point, the centralised critic dominates: it beats the equivalent shortest-path control plane at \emph{every} offered load under concentrated demand and wins every matched centralised-vs-local comparison, through a mechanism (longer paths, wider load spread) that is directly measurable in the forwarding statistics; a three-seed retraining campaign confirms this ordering is seed-stable, while showing that the within-family value-head choice is not --- so we rank the former and deliberately decline to rank the latter. Second, we introduce an honest baseline methodology for learned routing --- random path spreading and a greedy least-utilised-path heuristic evaluated on traffic-paired episodes --- and use it to locate the learned policy on a delivery/latency frontier and to quantify the adaptivity headroom that remains unclaimed. Third, we test the common claim that learned routing is brittle to topology change with a randomised link-failure severity sweep: the policy trained on the intact topology degrades gracefully while random spreading collapses, inverting the delivery ranking precisely when robustness matters. Fourth, we quantify the impact of GNN-based state preprocessing and of training-regime calibration: the GNN encoder costs delivery on the intact topology across every architecture tested, and remains a net cost under failure for most but not all of them, revealing that its effect is architecture-dependent rather than uniformly negative; training-regime and reward-weight calibration, by contrast, each contribute several points of delivery, and the resulting stress-trained architecture costs only a small, bounded amount of delivery at nominal load.
...
While existing research has made significant progress in applying DRL and MARL to routing optimisation, several gaps remain. (i)~\emph{Systematic evaluation of critic architectures:} MADDPG-based routing studies rarely compare degrees of critic centralisation under a fixed algorithmic family and environment; most adopt a centralised critic without investigating the trade-off. (ii)~\emph{GNN integration impact:} it remains unclear whether GNN state preprocessing alters the performance hierarchy between centralised and local critics, and whether its purported topology-awareness yields robustness to failures. (iii)~\emph{Honest baselines:} learned policies are compared against shortest path but not against uninformed randomisation or informed greedy heuristics, leaving both the true value and the remaining headroom of learning unquantified. (iv)~\emph{Resilience under topological change:} the claim that learned policies trained on one topology degrade badly when links fail is widely repeated but rarely tested with randomised, severity-controlled failures. (v)~\emph{Training-regime calibration:} the operating point at which the policy is trained is rarely matched to the regime where adaptivity is claimed to pay off. Our work addresses these gaps with a controlled comparison on a real service-provider topology, with stress-calibrated training, paired-traffic baselines, and randomised failure-severity evaluation.

A scoping choice should be stated explicitly: we compare architectures \emph{within} a single algorithmic family against strategy-space bounds, rather than reproducing published learned-routing systems as baselines. This is deliberate. Cross-paper comparisons of learned policies are confounded by exactly the factors this paper measures --- Section~\ref{sec:ablation} shows that training-regime and reward calibration alone are worth $+7.3$\,pp of delivery, more than most architectural effects --- as well as by differences in simulator, traffic model, and action space, so a reproduced external baseline would compare \emph{configurations}, not \emph{methods}. The fixed-rule baselines used here instead bound the achievable range on identical traffic: the greedy rule upper-envelopes per-step adaptive routing using exactly the information in the agents' observations, and random spreading lower-bounds what uninformed path diversity already buys. Any learned policy, from any paper, can be located against these same portable bounds --- which is precisely what a leaderboard of incomparable configurations cannot offer.
...
The framework is implemented in Python with PyTorch; the topology is represented as a NetworkX graph carrying per-link capacity and utilisation. A \texttt{NetworkEngine} advances a hop-by-hop, flow-based packet simulation: at each step every PE agent forwards queued packets along the selected $K$-path class, link utilisation is updated, and rewards are computed. One \emph{step} is a single synchronous forwarding round in which each in-flight packet advances at most one hop; the simulator carries no wall-clock time, so all delay figures (including the $95$th-percentile end-to-end delay $d_{95}$) are expressed in these step units --- effectively hop-latency counts --- and are meaningful only in relative comparison across strategies on the same traffic. Pre-computed $K$-shortest paths are refreshed whenever the topology changes (e.g.\ after a link failure) so that every routing strategy, learned or fixed, always selects among reachable routes.

The abstraction level of this simulator, and its consequences for the claims, should be stated plainly. The simulation is flow-based and step-clocked: it captures link capacities, queueing backlogs, congestion-driven loss, and hop-by-hop forwarding, but not transport-layer dynamics (no TCP congestion control or retransmission) and delay only in step units. Absolute delivery and delay figures would differ under a packet-level simulator or in deployment. The claims of this paper, however, are \emph{comparative}: every learned variant and every fixed rule runs on the identical engine, identical traffic realisations (paired episodes), and identical path sets, so the reported orderings and gaps measure differences between routing strategies, not simulator artefacts. Relative rankings of this kind are the quantities most likely to survive a change of simulation fidelity; the absolute numbers should be read as internally consistent rather than as deployment predictions.
...
\subsection{Scope: which properties of the topology carry the results}
\label{subsec:topology-scope}
Because every conclusion in this paper is drawn on one (real) network, we state explicitly which of its structural properties the headline behaviours rest on, and verify that our instance is representative of the service-provider \emph{class} rather than idiosyncratic. Two properties matter. \emph{(i)~Path redundancy:} multiple hop-diverse end-to-end routes exist between provider edges, a direct consequence of the hierarchical distribution/core design with meshed inter-cluster links; SP networks are engineered for exactly this redundancy, since it is also what makes fast reroute possible. \emph{(ii)~Capacity headroom:} the core is provisioned for peak demand, so even the most-loaded links retain slack (tail utilisation stays below $0.15$ even at $3\times$ hotspot load; Table~\ref{tab:mechanism}). Both are load-bearing: random spreading's nominal-load strength (Section~\ref{sec:baseline-frontier}) is variance reduction over near-disjoint spare-capacity paths, and the learned policy's graceful failure degradation (Section~\ref{sec:failure}) exploits surviving alternates that exist only because of (i).

% TODO(bib): add knight2011internet to Routing.bib --- M. Knight et al.,
% "The Internet Topology Zoo," IEEE JSAC 29(9):1765-1775, 2011.
To show that (i) is a class property and not an artefact of our instance, Table~\ref{tab:topo-zoo} measures it on seven real backbone/SP topologies from the Internet Topology Zoo~\cite{knight2011internet} alongside ours, using two structural statistics: the fraction of links that are \emph{bridges} (single points of failure, whose removal disconnects the graph) and the mean number of \emph{edge-disjoint paths} between core node pairs. Our topology falls inside the class range on every statistic (bridge fraction $0.20$ vs.\ a class range of $0.05$--$0.34$; mean edge-disjoint paths $2.06$ vs.\ $1.61$--$3.03$; mean degree $2.47$ vs.\ $2.3$--$3.9$). The robust, class-wide invariant is \emph{pervasive two-path redundancy}: bridges are a small minority of links ($13\%$ on average) and almost every core pair has at least two disjoint routes. Full three-way edge-disjointness is less common ($15\%$ of core pairs on average), which is precisely why our forwarding model uses $K{=}3$ \emph{hop-diverse} paths (sharing some links) rather than strictly disjoint ones --- a modelling choice that matches, rather than exceeds, the redundancy the class actually provides. Property (ii), capacity headroom, is a provisioning decision not encoded in the topology-only Zoo data, so we cannot measure it across the class; we report it for our instance and note that peak-provisioned cores are standard SP practice.

The conclusions should therefore transfer to topologies sharing these invariants --- the SP class broadly --- and should \emph{not} be extrapolated to sparse, capacity-tight, or tree-like access networks, where path diversity is structurally absent and none of the redundancy-driven mechanisms can operate. Assessing the value of learned routing \emph{in the wild}, on the network class it would actually be deployed in, is the point of this scoping; a full re-run of the comparison on a second SP instance to confirm the class-level performance claim is left for future work.

\begin{table}[!t]
  \centering
  \caption{Structural representativeness of the evaluation topology. Seven real backbone/SP networks from the Internet Topology Zoo~\cite{knight2011internet} vs.\ ours. Statistics computed on the largest connected component; edge-disjoint paths sampled over pairs of degree-$\geq2$ nodes in the $2$-core. Our instance falls inside the class range on every column.}
  \label{tab:topo-zoo}
  \small
...
\begin{itemize}
  ...
  \item \textbf{Greedy least-utilised}: at every decision, selects for each destination the path class whose current \emph{bottleneck} (maximum-over-hops) utilisation is lowest. This informed, myopic heuristic consumes exactly the $\mathbf{U}_i$ signal available in the agents' observations. It is \emph{not} a flow-optimal oracle --- being memoryless and uncoordinated it can herd traffic under contention, and Section~\ref{sec:failure} shows the learned policy overtaking it under severe failures --- but on the intact topology it traces a strong upper envelope of per-step adaptive routing. Because a globally coordinated multicommodity-flow optimum would lie at or above the greedy envelope, the adaptivity-headroom figures we derive from it are \emph{conservative lower bounds} on the delivery a perfectly-informed router could achieve; the true gap left by the learned policy is at least as large as reported.
...
\label{sec:ceiling}
Figure~\ref{fig:ceiling} compares all seven variants on 20 traffic-paired episodes at the $2\times$ hotspot operating point, now with 95\% confidence intervals (normal approximation on the paired-episode standard deviation, $n=20$) on each variant's PDR estimate, against the fixed-rule marks on the same traffic. The critic-domain ordering is consistent with the load sweep and statistically resolved even at this single operating point: CC-Simple's interval ($87.3\pm1.6$) does not overlap any other variant's, and every centralised variant's point estimate exceeds every local variant's. The value-head comparison is more nuanced than a single point estimate suggests: on this seed the simple head appears to beat duelling in the centralised family ($87.3\pm1.6$ vs.\ $82.3\pm1.8$, non-overlapping CIs), while in the local family the two heads are statistically indistinguishable ($77.2\pm2.3$ vs.\ $76.6\pm2.5$, heavily overlapping). We caution that the centralised-family gap does \emph{not} survive the multi-seed campaign (Section~\ref{sec:seed-variance}): across three seeds the CC-Simple/CC-Duelling ordering reverses, so we report all within-family value-head comparisons directionally only. Every GNN variant trails its non-GNN base; Section~\ref{sec:gnn} gives paired, higher-power evidence for this comparison. The span of the fixed rules on identical traffic --- from $65.9\%$ (worst) to $97.2\%$ (greedy) --- confirms that path selection is worth $31$\,pp of delivery at this operating point, of which the best learned policy captures about two thirds above the worst-case floor ($+21.4$\,pp).

Two practical conclusions follow. The critic domain is worth $\sim$$10$\,pp (CC-Simple vs.\ LC-Simple) --- by far the dominant design choice, and the only one that is statistically resolved \emph{and} seed-stable (Section~\ref{sec:seed-variance}). The value head's effect, by contrast, is within the training-seed noise: the apparent single-seed advantage of the simple head in the centralised family does not persist across seeds, so we do not claim a value-head ordering.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{F7_variant_ceiling_2x.pdf}
  \caption{End-to-end PDR of all seven variants on 20 traffic-paired episodes at $2\times$ hotspot, with 95\% CIs (normal approximation) and the fixed rules (vertical lines, labelled above the axes) evaluated on the same traffic. Hatched bars are GNN variants. CC-Simple's CI does not overlap any other variant's, and every centralised variant exceeds every local one; the apparent CC-family simple-over-duelling gap on this seed does not survive the multi-seed campaign (Section~\ref{sec:seed-variance}).}
  \label{fig:ceiling}
\end{figure}

\subsection{Training-seed variance}
\label{sec:seed-variance}
To separate genuine architectural effects from training-seed noise, the four non-GNN variants were retrained from scratch under two further independent seeds (in addition to the canonical run used above) and re-evaluated on the identical paired ceiling protocol at $2\times$ hotspot. Figure~\ref{fig:seedvar} shows the resulting clean-delivery estimates; the policy-independent worst-path rule evaluates to $65.8\%$ on the shared traffic under every seed, a consistency check that the evaluation harness is seed-invariant where it should be. Two findings result. First, the \emph{critic-domain ordering is seed-stable}: CC-Simple exceeds LC-Simple by $+7.5\pm2.3$\,pp (mean over three paired seeds), and the gap is positive in every seed ($+10.1, +6.2, +6.0$\,pp), so the $\sim$$10$\,pp headline advantage of the centralised critic is not a seed artefact. Second, the \emph{value-head ordering is not resolved}: the CC-Simple$-$CC-Duelling difference is $+2.0\pm2.8$\,pp and \emph{changes sign} across seeds ($+5.0, +1.7, -0.7$\,pp), and the LC pair is likewise within noise ($+0.5\pm0.9$\,pp). The per-variant three-seed means are CC-Simple $86.0\pm1.5$, CC-Duelling $84.0\pm1.5$, LC-Duelling $79.0\pm2.1$, and LC-Simple $78.5\pm1.4$; with only three seeds we also report the min--max envelopes, which do not close the family gap: CC-Simple $[84.4, 87.3]$ and CC-Duelling $[82.3, 85.1]$ both sit entirely above LC-Duelling $[76.7, 80.7]$ and LC-Simple $[77.2, 80.0]$. The two critic families are cleanly separated, the two heads within each family are not. This is the intended outcome of the multi-seed check---it confirms the claim the paper actually rests on (critic domain) and retracts the one it does not need (value head).

\begin{figure}[t]
  \centering
  \includegraphics[width=0.9\linewidth]{F12_seed_variance.pdf}
  \caption{Clean end-to-end PDR at $2\times$ hotspot for the four non-GNN variants across three independent training seeds (points; $20$ paired episodes per seed-point on the ceiling protocol), with three-seed mean $\pm$ s.d.\ (bars). The centralised family (blue) separates from the local family (orange) in every seed; the two value heads within each family overlap. Dashed line: the policy-independent worst-path floor ($65.8\%$), identical across seeds.}
  \label{fig:seedvar}
\end{figure}
...
These results offer concrete guidance: deploy a centralised-critic agent---the dominant, seed-stable design choice, for which the simpler value head suffices---trained at the stress regime where adaptivity is needed; benchmark it against random spreading and an informed greedy rule, not only shortest path; and evaluate robustness with randomised, severity-controlled failures. Several limitations frame future work. The claims are scoped to the service-provider topology class whose structural invariants --- hop-diverse paths and provisioned capacity headroom --- carry the results (Section~\ref{subsec:topology-scope}); a second SP instance would confirm the class-level claim, and capacity-tight or diversity-poor networks are explicitly out of scope. The multi-seed variance characterisation covers the four non-GNN variants (Section~\ref{sec:seed-variance}) but not yet the three GNN variants, so the single CC-Duelling failure-time GNN gain remains provisional. Finally, the unclaimed random-to-greedy headroom motivates architectures and objectives that can close the learnability gap (e.g.\ stochastic policies that subsume spreading), and GNN encoders with inference-time graph reconstruction remain untested. The same architectural comparison under adversarial perturbation of the agents' observations is the subject of a companion study, assessing whether the advantages identified here persist under malicious, rather than merely natural, stress.

\section*{Acknowledgements}
% TODO (before submission): replace the two bracketed placeholders below with the
% actual IT project name/number and FCT grant number — placeholders are a
% desk-reject risk if they slip through.
