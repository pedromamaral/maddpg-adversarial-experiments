...
Because every conclusion in this paper is drawn on one (real) network, we state explicitly which of its structural properties the headline behaviours rest on, and why these are properties of the service-provider \emph{class} rather than of the instance. Two matter. \emph{(i)~Path diversity:} the $K$-shortest-path procedure finds $K{=}3$ hop-diverse end-to-end routes for every PE pair, a direct consequence of the hierarchical distribution/core design with meshed inter-cluster links; SP networks are engineered for exactly this redundancy, since it is also what makes fast reroute possible. \emph{(ii)~Capacity headroom:} the core is provisioned for peak demand, so at nominal load even the most-loaded links retain slack (tail utilisation remains below $0.15$ even at $3\times$ hotspot load; Table~\ref{tab:mechanism}). Both properties are load-bearing for the results: random spreading's nominal-load strength (Section~\ref{sec:baseline-frontier}) is variance reduction over near-disjoint spare-capacity paths, and the learned policy's graceful failure degradation (Section~\ref{sec:failure}) exploits surviving alternatives that only exist because of (i). The conclusions should therefore transfer to topologies sharing these invariants --- i.e.\ to the SP class broadly, which is provisioned this way by design --- and should \emph{not} be extrapolated to sparse, capacity-tight, or tree-like access networks, where path diversity is structurally absent and none of the redundancy-driven mechanisms can operate. Assessing the value of learned routing \emph{in the wild}, on the network class it would actually be deployed in, is the point of this scoping; re-running the comparison on a second SP instance to confirm the class-level claim is left for future work.
